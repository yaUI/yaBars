...
local function updatehotkey(self, actionButtonType)
	...
	key = string.gsub(key, '(s%-)', 'S')
	key = string.gsub(key, '(a%-)', 'A')
	key = string.gsub(key, '(c%-)', 'C')
	...
	key = string.gsub(key, '(Page Up)', 'PU')
	key = string.gsub(key, '(Page Down)', 'PD')
	key = string.gsub(key, '(Spacebar)', 'SpB')
